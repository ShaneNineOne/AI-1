...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        return (self.startingPosition, [])#Starting position, list of visited corners
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        if state[0] in self.corners and state[0] not in state[1]:#If the position is a corner and not in our list of visited corners
            state[1].append(state[0])
        if len(state[1]) == 4:#If the length of our visited corners is equal to 4
            return True
        return False
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            hitsWall = self.walls[nextx][nexty]
            if not hitsWall:
                next = (nextx, nexty)#Next state is equal to the (next x, next y position)
                successor = ((next, list(state[1])), action, 1)#Successor = next state, action, and the cost
                successors.append(successor)#Append the successor to our list of successors.
        return successors
